...
class ListEntryViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
    ...
    @IBAction func enterUnitsTextField(_ sender: UITextField) {
        unitsTextField.resignFirstResponder()
        unitsPicker.resignFirstResponder()
    }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        self.unitsTextField.endEditing(true)
    ...
    @IBAction func saveEntry(_ sender: UIButton) {
        if entryTextField.text!.isEmpty {
            ...
            item.id = UUID()
            item.orderPosition = Int32(countEntries!)
